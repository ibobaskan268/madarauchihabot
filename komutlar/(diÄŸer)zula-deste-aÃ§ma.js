...
exports.run = (client, message) => {
    ...
    var botclub= botclub[Math.floor(Math.random(1) * botclub.length)]
    const botklüp= new Discord.RichEmbed()
    ...
    .setDescription(`${botclub}`)
    .setFooter(`Desteyi açan (${message.author.username}) unutma dostum, bu gerçek hesabına gelmez.`)
    .setColor("RANDOM")
    return message.channel.sendEmbed(botklüp);
};
exports.conf = {
  enabled: true,
  guildOnly: false,
